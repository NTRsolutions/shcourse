...
class Work extends CI_Controller {
	...
	public function view($content_id)////$id is content id
    {
        ///we have to increase view num to contents.
            ...
            if ($content_type_id == '1') {///script work
                ...
                $this->data['scriptText'] = $this->get_scriptDetails($contentInfo->file_name);

            } else if ($content_type_id == '2') {///dubbing work

                $this->data['scriptWavPath'] = $contentInfo->file_name.'_song.wav';

            } else if ($content_type_id == '3') {//head mask work

                $this->data['headImagePath'] = $contentInfo->file_name;

            } else if ($content_type_id == '4') {////shooting

                $this->data['videoPath'] = $contentInfo->file_name;
            }
            $this->data['content_type_id'] = $content_type_id;
            if($content_type_id=='4'){
             $this->data["subview"] = "work/content_shooting";
            }else if($content_type_id=='2') {
             $this->data["subview"] = "work/content_dubbing";
            }else{
                $this->data["subview"] = "work/content";
            }
    ...
    public function head($user_id)
    {
        if($this->signin_m->loggedin()) {
            $this->data['user_id'] = $user_id;
            $this->data['fullname'] = $this->users_m->getFullNameFromUserId($user_id);///current student name;
            $this->data['contents'] = $this->contents_m->get_work($user_id, '3');
            $this->data['content_type_id'] = '3';
    ...
    public function shooting($user_id)
    {
        if($this->signin_m->loggedin()) {
            $this->data['user_id'] = $user_id;
            $this->data['fullname'] = $this->users_m->getFullNameFromUserId($user_id);///current student name;
            $this->data['contents'] = $this->contents_m->get_work($user_id,'4');
            $this->data['content_type_id'] = '4';
    ...
    public function student()///this function is for teacher of front end
    {
        if($this->signin_m->loggedin()) {
            ...
            $this->data["subview"] = "work/student";
            $this->load->view('_layout_main', $this->data);
        }else{
            redirect(base_url().'signin/index');
        }
    ...
    public function getClassList($classListStr)///this function will be used for teachers
    {
        ...
        if($classListStr!='')
        {
            ...
            foreach ($classArr as $class_info):
                ...
                $tempObj->image_name =  $gradeNo.'-'.$classNo.'.png';
...
//              $ret['data'] = $this->output_content($this->data['contents'],$usingStatus);
    ...
    public function output_content($contents,$usingStatus)
    {
        ...
        foreach ($contents as $content):
           ...
           if($usingStatus) {///current user is student
               ...
               $output .= '<a href="' . base_url('work/view') .$content->content_id . '">';
    ...
    public function getMembers()
    {
        ...
        if($_POST)
        {
            if($this->session->userdata()) {
                ...
                //$ret['data'] = $this->output_members($users,$content_type_id);
    ...
    public function output_members($users,$content_type_id)
    {
        ...
        $output = '';
        $output .= '<tr>';
        $output .= '<td><a href="'.base_url().$studentURL.$user_id.'">'.$this->lang->line('Me');
        $output .=  '</a>';
        $output .=  '</td>';
        $output .= '</tr>';
        ...
        foreach ($users as $user):
            if($this->users_m->hasContents($user->user_id,$content_type_id)){
                ...
                $output .='<td><a href="'.base_url().$studentURL.$user->user_id.'">'.$user->fullname.'</a></td>';
                $output .='</tr>';
            }
        endforeach;
        return $output;
    ...
    public function output_members_custom($users,$content_type_id)
    {
        ...
        $output .=         '<a href="'.base_url().$studentURL.$user_id.'" class="member_item">'.'&nbsp&nbsp'.$this->lang->line('Me').'</a>';
        $output .=      '</div>';
        ...
        foreach ($users as $user):
            //if($this->users_m->hasContents($user->user_id,$content_type_id)){
                ...
                if($extraCounts==0){
                    ...
                    $output .=     '<div class="member_item_wrapper">';
                    $output .=         '<a href="'.base_url().$studentURL.$user->user_id.'" class="member_item" >'.($serialNo+1).'&nbsp'.$user->fullname.'</a>';
                    $output .=      '</div>';
