...
class Community extends CI_Controller {
	...
	public function index(){
        ...
        if($this->signin_m->loggedin()) {
            ...
            $this->data["subview"] = "community/index";
    ...
    public function view($content_id)//this function show comment page
    {
        ...
        if($this->signin_m->loggedin()) {
            ...
            if ($content_type_id == '1') {///script work
                ...
                $this->data['scriptWavPath'] = $contentInfo->file_name;
            ...
            if($content_type_id=='4'){
                $this->data["subview"] = "community/content_shooting";
            }else if($content_type_id=='2'){
                $this->data["subview"] = "community/content_dubbing";
            }else{
                $this->data["subview"] = "community/content";
            }
    ...
    function increase_voteNum()
    {
        ...
        {
            $content_id = $_POST['content_id'];
            $updatedVoteNum = $this->contents_m->update_vote_num($content_id);
    ...
    public function output_content($contents)
    {
        ...
        foreach ($contents as $content):
            $output .= '<tr>';
            $output .= '<td>';
            $output .=  '<a href = "'.base_url().'community/view/'.$content->content_id.'">';
            $output .=  '【'.$content->content_type_name.'】'.$content->content_title;
            $output .=  '</a></td>';
            $output .=  '<td><a href = "';
            $output .=  base_url().'users/profile/'.$content->content_user_id.'">';
    ...
    public function output_custom_content($contents)
    {
        ...
        foreach ($contents as $content):
            ...
            $tempObj->content_url = base_url().'community/view/'.$content->content_id;
            $tempObj->profile_url = base_url().'users/profile/'.$content->content_user_id;
