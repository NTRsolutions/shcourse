...
class Schools extends Admin_Controller
{
    ...
    public function output_content($schools)
    {
        ...
        foreach( $schools as $school):
                ...
                foreach ($classArr as $class_info):
                    ...
                    for($i=1;$i<=$class_info->class;$i++)
                    {
                        ...
                        if($i==$class_info->class){
                            ...
                            $output .= $realStr.$gradeStr.$realClassStr.$classStr.$seperator;
                        }
                    }
                    $output .= '<br/>';
                endforeach;
            $output .= '</td>';
            $output .= '<td align="center" style=" vertical-align:middle">';
            $output .=  '<button class="btn btn-sm btn-success" onclick="edit_school(this);" school_id = '.$school->school_id.'>'.$this->lang->line('Modify').'</button>';
