...
var imgData = new Array(
    ...
    { name: 'hd3_bg', path: 'images/script-bg.png' },
    { name: 'hd3_q', path: 'images/question.png' },
...
var GameHd3 = {
    _data:{},
    createNew:function(){
        var game = GameClass.createNew("Hd3", {});
        game._pInit = game.init;
        var bg;
        game.init = function(){
            this._pInit();

            bg = GameClass.images.getImg('hd3_bg');
            this.bgLayer.addChild(bg);

            btn = new LButton(GameClass.images.getImg("hd3_q"));
            btn.x = 58;
            btn.y = 338
            btn.width = 130;
            btn.height = 130;
            this.btnLayer.addChild(btn);
            btn.addEventListener(LMouseEvent.MOUSE_UP, function () {
                $('#head-file').trigger('click');

                $('#savehd_btn').click(function(){
                    head_img.result({
                        type: 'base64'
                        ...
                        head_base64 = base64;
                        head_img.destroy();
                        clearFileInput(document.getElementById("head-file"));
                        $('#imgcropModal').modal('hide');
                        game.selectHead(base64);
                    });

                })

                $('#cloasehd_btn').click(function(){
                    if( head_img != undefined && head_img != null )
                        head_img.destroy();
                    clearFileInput(document.getElementById("head-file"));
                    $('#imgcropModal').modal('hide');
                });
        ...
        game.selectHead = function(base64){

            //var tempImage = new Image();
            //console.log(base64);
            //tempImage.src = base64;
            //btn.bitmap_over.bitmapData = new LBitmapData(tempImage, 0, 0, 130, 130, LBitmapData.DATA_CANVAS);

            loader = new LLoader();
            loader.addEventListener(LEvent.COMPLETE,loadBitmapdata);
            loader.load(base64,"bitmapData");
            function loadBitmapdata(event) {
                btn.bitmap_over.bitmapData = new LBitmapData(loader.content, 0, 0, 130, 130);
                console.log(btn.bitmap_over.bitmapData.width);
                console.log(btn.bitmap_over.bitmapData.height);
                var scalex = 130/btn.bitmap_over.bitmapData.width;
                var scaley = 130/btn.bitmap_over.bitmapData.height;
                btn.scaleX = scalex;
                btn.scaleY = scaley;
            }
        }
