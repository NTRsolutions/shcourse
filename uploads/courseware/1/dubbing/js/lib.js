...
(function(lib){
	...
	function onresize(e) {
		...
		var scale1 = Math.min(1135/1280,1280) * ratio;
		...
		$('.content').css({width:Math.round(1280*scale*scale1),height:Math.round(720*scale*scale1),'margin-left':Math.round(24*scale), 'margin-top':Math.round(38*scale)});
		...
		$('#game_canvas').css({width:1270,height:667,margin:0});
		$('#game_canvas').attr({width:1270,height:667});
		...
		try {
			...
			vplayer.height(Math.round(715*scale*scale1));
		}catch(err){
...
function showPart(id) {
	...
	switch(id) {
		...
		case '2':
			currentID = id;
			if(currentID == id) showVideo("video/tomorrow002.mp4",function(){
				//console.log('end')
				//if(currentID == id) GameClass.startGame('GameHdend');
			});
			break;
		case '3':
			cur_stage = 'script';
			currentID = id;
			if(currentID == id) GameClass.startGame('GameHd3');
			break;
	}
}
